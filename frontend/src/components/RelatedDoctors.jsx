...
const RelatedDoctors = ({speciality,docId}) => { //we take the props that we have passed here

    const {doctors}=useContext(AppContext) //we get all doctors from context file

    const [relDoc,setRelDocs]=useState([])

    const navigate=useNavigate() //for navigation

    useEffect(()=>{  //we use useeffect to change relDocs
        if(doctors.length>0 && speciality)
        {
            const doctorsData=doctors.filter((doc)=>doc.speciality===speciality && doc._id!==docId)
            //this variable stores data of doctors whose speciality matches that of the cuurrent doc
            //first filter doctors and store in the related doctors array
