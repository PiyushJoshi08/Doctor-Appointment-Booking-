//to authenticate the token of a  doctor to act as a middleware


import jwt from 'jsonwebtoken'


const authDoctor = async(req,res,next)=>{ //next is a callback function
    try{
        //if we have token in header then only we allow api call tobe  done

        const {dtoken}=req.headers //doctor token is token taken from headers of req
        ...
        const token_decode= jwt.verify(dtoken,process.env.JWT_SECRET); //this variable has decoded token which is email_+pswd
